<template>
	<view class="uni-table-scroll" :class="{ 'table--border': border, 'border-none': !noData }">
		<!-- #ifdef H5 -->
		<table class="uni-table" border="0" cellpadding="0" cellspacing="0" :class="{ 'table--stripe': stripe }" :style="{ 'min-width': minWidth + 'rpx' }">
			<slot></slot>
...
.table--border {
	border: 2rpx $border-color solid;
	border-right: none;
}
...
.empty-border {
	border-right: 2rpx $border-color solid;
}
.uni-table-text {
	position: absolute;
	right: 0;
